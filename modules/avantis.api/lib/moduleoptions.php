...
class ModuleOptions
{
    ...
    public function __construct($module_id, $arTabs, $arGroups, $arOptions, $need_access_tab = false)
    {
        ...
        if($_REQUEST['update'] == 'Y' && check_bitrix_sessid()){
            ...
            {
                global $APPLICATION;
                LocalRedirect($APPLICATION->GetCurUri() . '&tabControl_active_tab=' . $_REQUEST['back_url']);
            }
        }
    ...
    private function SaveOptions()
    {
        foreach ($this->arOptions as $opt => $arOptParams) {
            if ($arOptParams['TYPE'] != 'CUSTOM') {
                if ($arOptParams['TYPE'] === 'CUSTOM_SELECTBOX') {
                    $params = [
                        $opt,
                        '',
                        "",
                        array("selectbox", $arOptParams['VALUES']),
                        "",
                        "",
                        "Y"
                    ];
                    __AdmSettingsSaveOption($this->module_id, $params);
                    continue;
                }
                ...
                if ($arOptParams['TYPE'] == 'SELECT' && $arOptParams['MULTIPLE'] == 'Y' )
                {
                    $newVal = [];
                    ...
                    $val = serialize(array_filter($newVal));
                }
    ...
    private function GetCurOptionValues()
    {
        foreach($this->arOptions as $opt => $arOptParams)
        {
            if($arOptParams['TYPE'] != 'CUSTOM')
            {
                ...
                if(in_array($arOptParams['TYPE'], array('MSELECT', 'MSTRING')) || ($arOptParams['TYPE'] == 'SELECT' && $arOptParams['MULTIPLE'] == 'Y'))
    ...
    public function ShowHTML()
    {
        ...
        if(is_array($this->arOptions))
        {
            foreach($this->arOptions as $option => $arOptParams)
            {
                ...
                switch($arOptParams['TYPE'])
                {
                    ...
                    case 'MSTRING':
                        ...
                        $desc_arr = array(
                            'NAME' => $label,
                            'ACTIVE' => 'Y',
                            'DEFAULT_VALUE' => $arOptParams['DEFAULT'],
                            'PROPERTY_TYPE' => 'S',
                            'MULTIPLE' => 'Y',
                            'MULTIPLE_CNT' => $arOptParams['MULTIPLE_CNT']
                        );
                        ob_start();
                        _ShowPropertyField(htmlspecialchars($option), $desc_arr, $val,false, false, 50000, $this->module_id);
                        $input = ob_get_clean();
                        if($arOptParams['REFRESH'] == 'Y')
                            $input .= '<input type="submit" name="refresh" value="OK" />';
                        break;
                    case 'CUSTOM_SELECTBOX':
                        $params = [
                            $option,
                            '',
                            "",
                        ...
                        ob_start();
                        __AdmSettingsDrawRow($this->module_id, $params);
                ...
                $arP[$this->arGroups[$arOptParams['GROUP']]['TAB']][$arOptParams['GROUP']]['OPTIONS'][] = $label != '' ? '<tr><td valign="top">'.$label.'</td><td valign="top" nowrap>'.$input.'</td></tr>' : '<tr><td valign="top" colspan="2" align="center">'.$input.'</td></tr>';
            ...
            echo "<script>
                    BX.addCustomEvent('onAdminTabsChange', function(){
                         var activeTab = BX('tabControl_active_tab');
                         var inputBackUrl = document.querySelector('input[name=\"back_url\"]');
                         if(inputBackUrl && activeTab){
                            inputBackUrl.value =  activeTab.value;
                         }   
                       });
                   </script>";
        }
